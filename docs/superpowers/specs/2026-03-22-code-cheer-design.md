...
| 优先级 | 触发条件 | 实现方式 |
|--------|---------|---------|
| 1（最高） | 用量突破阈值（80% / 95%） | 检查 `last_rate_tier` 是否跨级，立即换词 |
| 2 | 每次命令执行完成 | `PostToolUse` hook 调用 `statusline.py --update` |
| 3 | 时段切换（晨/午/晚/深夜） | 对比当前时段与 `state.json` 中上次时段 |
| 4（兜底） | 缓存超过 5 分钟 | 对比 `last_updated` 时间戳，随机选不重复词条 |
...
`last_rate_tier` 取值：`normal` / `warning`（≥80%）/ `critical`（≥95%）
...
- `$ARGUMENTS` 为空 → 用 `AskUserQuestion` 展示四个角色（含 ASCII 表情和风格说明），用户点选后切换
- `$ARGUMENTS` 为有效角色名（nova/luna/mochi/iris）→ 直接切换
- `$ARGUMENTS` 无效 → 提示可用角色列表

**切换步骤：**
1. 写入 `~/.claude/code-cheer/config.json`：`{"character": "<name>"}`
2. 清空 `state.json` 缓存，新角色立即在状态栏出现
3. 用该角色风格回复一条确认语
...
#!/bin/bash
# 1. 检查 Python 3 可用性
# 2. 创建 ~/.claude/code-cheer/ 目录
# 3. 复制脚本、core/、vocab/ 到 ~/.claude/code-cheer/
# 4. 写入默认 config.json：{"character": "nova"}
# 5. 写入空 state.json
# 6. 注入 ~/.claude/settings.json：
#    - statusLine: {"type":"command","command":"python3 ~/.claude/code-cheer/statusline.py"}
#    - hooks.PostToolUse: {"command": "python3 ~/.claude/code-cheer/statusline.py --update"}
# 7. 复制 commands/cheer.md → ~/.claude/commands/cheer.md
# 8. 打印安装成功信息（Nova 风格欢迎语）
```

支持 `./install.sh --uninstall` 清理所有写入项并还原 `settings.json`。
...
def resolve_message(character, state, stats, cc_data):
    used_pct = cc_data["rate_limits"]["used_percentage"]
    ...
    # 3. 时段切换 → 时段词
    slot = get_time_slot()
    if slot != get_time_slot(state["last_updated"]):
...
| 角色配置 | `~/.claude/code-cheer/config.json` | 持久化，与斜杠命令联动 |
| 状态缓存 | `~/.claude/code-cheer/state.json` | 避免状态栏频繁闪烁换词 |
